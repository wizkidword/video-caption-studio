...
def test_strict_mode_fails_with_runtime_reason(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (
            False,
            "Transcript runtime failed: required whisper/runtime libraries were not found.",
            "Could not load ctranslate2.dll",
        ),
    )
    ...
    msg = str(exc.value)
    assert "Transcript runtime failed" in msg
    assert "Details: Could not load ctranslate2.dll" in msg
...
def test_strict_mode_passes_without_transcript_if_no_audio(monkeypatch):
    ...
    assert result.report.transcript_runtime_ok is True
    assert "no audio stream" in result.report.transcript_runtime_message.lower()
    assert any("No audio track detected" in w for w in result.report.warnings)


def test_fallback_mode_reports_runtime_failure_detail(monkeypatch):
    metadata = VideoMetadata(
        path="/tmp/fake.mp4",
        filename="fake.mp4",
        extension=".mp4",
    ...
    monkeypatch.setattr(
        analyze,
        "transcript_runtime_precheck",
        lambda *_args, **_kwargs: (False, "Transcript runtime failed while decoding audio from the video file.", "ffmpeg decode error"),
    )
    ...
    assert result.report.transcript_source == "none"
    assert result.report.transcript_dependency_available in (True, False)
